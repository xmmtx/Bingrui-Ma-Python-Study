...
x=100
y=400
pop=pygame.mixer.Sound('D:\\ball\\pop.wav')
...
while running:
    ...
    if picY>=500-picH:
        blap.play()
        speedY=-speedY
    if picY>=400-picH and speedY>0:
        if picX+picW/2>=x and picX+picW/2<=x+picDangBanW:
            print('小球x坐标'+str(picX+picW//2)+',挡板x坐标：'+str(x))
            pop.play()
            speedY=-speedY
    picX+=speedX
    picY+=speedY
