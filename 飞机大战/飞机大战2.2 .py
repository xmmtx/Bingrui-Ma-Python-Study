        ...
        for enemyPlane in enemyPlaneList:
            for bullet in enemyPlane.bullet_list:
                if bullet.x>self.x and bullet.x<self.x+width and bullet.y>self.y and bullet.y<self.y+height:
                    ...
                    self.bomMusic.play()
        for enemyPlane in enemyPlaneList:
            width1=enemyPlane.image.get_width()
            height1=enemyPlane.image.get_height()
            if self.x>=enemyPlane.x and self.x<=enemyPlane.x+width1 and self.y<=enemyPlane.y+height1 and self.y+76>=enemyPlane.y or self.x+106<=enemyPlane.x+width1 and self.x+106>=enemyPlane.x and self.y<=enemyPlane.y+height1 and self.y+76>=enemyPlane.y:
                enemyPlane.hit=True
                self.hit=True
                self.bomMusic.play()
    def startHero(self):
...
def key_control(hero_temp):
    ...
    if pressed_keys[K_SPACE]:
        print('space')
        hero_temp.fire()
def pauseGame(screen_temp):
    font=pygame.font.Font('./fonts/myfont.ttf',30)
    tip=font.render('游戏已暂停，按“S”键恢复游戏!',True,(255,0,0))
    screen_temp.blit(tip,((512-10*30)/2,(568-30)/2))
    ...
    while run:
        ...
        if pressed_keys[K_s]:
            return
def showStartGame(screen_temp):
    gameStartImg=pygame.image.load('./images/startGame.png')
    width=gameStartImg.get_width()
    height=gameStartImg.get_height()
    screen_temp.blit(gameStartImg,((512-width)/2,(568-height)/2))
    font=pygame.font.Font('./fonts/myfont.ttf',30)
    tip=font.render('按S键开始游戏!', True, (255, 0, 0))
    screen_temp.blit(tip,((512-width)/2,(568-height)/2+height))
    pygame.display.update()
    run=True
    while run:
        for event in pygame.event.get():
        ...
        if pressed_keys[K_s]:
            return
def showHreoLives(screen_temp,lives):
    font=pygame.font.Font('./fonts/myfont.ttf',12)
    tip=font.render('飞机命数：'+str(lives),True,(0,0,255))
    screen_temp.blit(tip,(10,568-20))
def showGameOver(screen_temp):
    font=pygame.font.Font('./fonts/myfont.ttf',30)
    tip=font.render('您输了，按R键重新开始游戏！',True,(255,0,0))
    screen_temp.blit(tip,((512-10*30)/2,(568-30)/2))
def main():
    pygame.init()
    ...
    while run:
        ...
        if m>=0:
            m=-968
        screen.blit(background,(0,m))
        showHreoLives(screen,hero.lives)
        # key_control(hero)
        if hero.lives>0:
            key_control(hero)
        hero.display()
